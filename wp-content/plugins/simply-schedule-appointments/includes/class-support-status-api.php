...
class SSA_Support_Status_Api extends WP_REST_Controller {
	...
	public function get_export_code( WP_REST_Request $request ) {
		...
		if( isset( $params['appointment_types'] ) && $params['appointment_types'] === 'true' ) {
			$payload['staff'] = $this->plugin->staff_model->query( array(
				'number' => -1,
				'order' => 'ASC',
			) );
			...
			$payload['staff_appointment_types'] = $this->plugin->staff_appointment_type_model->query( array(
				'number' => -1,
				'order' => 'ASC',
			) );
		}
		if( isset( $params['appointments'] ) && $params['appointments'] === 'true' ) {
			...
			$payload['staff_appointments'] = $this->plugin->staff_appointment_model->query( array(
				'number' => -1,
				'order' => 'ASC',
			) );
		} else if ( isset( $params['appointment_types'] ) && $params['appointment_types'] === 'true' ) {
